...
/*   Updated: 2022/11/09 13:12:47 by jinholee         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	check_errors(int argc, char *argv[])
{
	...
	dup_check = parse_input(argc, argv);
	sort_check = parse_input(argc, argv);
	if (is_duplicate(dup_check))
		ft_exit("Error\n", 1);
	if (is_sorted(sort_check))
		exit(0);
	clear(dup_check);
	clear(sort_check);
}
